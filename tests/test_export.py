...
def _plan(clips, **kw):
    return ExportPlan(
        ...
        video_basename=kw.get("base", "clip.mkv"), fps=kw.get("fps", 59.94),
        is_drop=kw.get("is_drop", True), out_path="out.edl",
        ntsc_note="", total_seconds=sum(c.duration for c in clips),
    )
